        ...
        if (request.status == 200){
            ...
            if (jsondata.adjacencyList.length > 0) {
                ...
                google.visualization.events.addListener(chart, 'select', function() {
                    ...
                    if (selection[0] != null) {
                        params = selection[0].name.split(" ");
                        var courseService = new XMLHttpRequest();
                        var courseURL = "http://localhost:8080/courses?subject=" + params[0] + "&number=" + params[1];
                        console.log(courseURL);
                        courseService.open("GET", courseURL, true);
                        courseService.send(null);
                        courseService.onload = () => {
                            if (courseService.status == 200) {
                                coursedata = JSON.parse(courseService.response);
                                console.log(coursedata);
                                modal = document.getElementById('classmodal');
                                modal.classList.add("is-active");
                                modalTitle = document.getElementById("coursetitle");
                                modalTitle.innerText = coursedata.attribute.subject + " " + coursedata.attribute.number + " " +
                                    coursedata.attribute.title;
                                modalMessage = document.getElementById("messagearea");
                                modalMessage.innerText = coursedata.attribute.description;
                            }
                        }
                    }
                });
